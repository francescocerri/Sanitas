...
4. [ ] **API: richiesta di prenotazione (volontario)**: nuovo endpoint che crea una prenotazione "in attesa" per il chiamante stesso (`volunteer_id` preso dal JWT, non dal body — cambio deliberato rispetto al contratto odierno di `POST /v1/shifts`), verificando ruolo e disponibilità dello slot. Espone anche il conteggio delle richieste in sospeso, base del contatore in-app per il gestore turni.
