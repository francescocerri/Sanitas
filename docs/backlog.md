...
### Gestione turni (sostituisce "Progettazione del dominio reale")

Suddivisione in attività autonome, ciascuna con la propria sessione di Plan Mode dedicata quando ripresa, a partire dalla prima che definisce lo schema condiviso dalle altre. Notifiche al gestore turni: in-app da subito (attività 4), email in un'attività separata e successiva (attività 8) — decisione presa esplicitamente, non da rivedere senza motivo. Prenotazioni multiple del volontario: endpoint bulk atomico (attività 7), non più chiamate singole.

1. [x] **ADR + ridisegno del modello dati turni**: la vecchia tabella piatta è sostituita da due tabelle — `shift_templates` (turni ricorrenti: giorno della settimana + orario + attivo/non attivo, impostati dal gestore turni da app) e `bookings` (una riga per volontario+data+slot, con stato `pending`/`confirmed`/`rejected`/`cancelled` e una copia "fotografia" degli orari presa dal template al momento della richiesta). Nuovo permesso `shifts:configure` per decidere giorni/orari dei turni-template, assegnato **solo** a `shift_manager` (non a `president`, a differenza di `shifts:write`). Vedi [ADR-0025](adr/0025-modello-dati-turni.md).
2. [x] **API: gestione turni-template (gestore turni)**: `POST`/`GET`/`PATCH /v1/shift-templates` (creazione e modifica dietro `shifts:configure`, lettura dietro `shifts:read` — la aprono a chiunque, non solo al gestore, perché anche i volontari devono poter vedere quali turni esistono). Seed iniziale per Pavullo da `config/pavullo/shifts/templates.json`, applicato una sola volta se la tabella è vuota (le modifiche del gestore turni da app non vengono più sovrascritte ai riavvii successivi).
3. [x] **API: calendario e copertura (sola lettura)**: `GET /v1/shift-occurrences?from=...&to=...` (dietro `shifts:read`, intervallo massimo 90 giorni) — elenca le occorrenze concrete dei turni-template attivi nell'intervallo, generate al volo (non una tabella), con lo stato di copertura (`free`/`pending`/`confirmed`, calcolato dalle prenotazioni esistenti — `confirmed` vince su `pending`). Base dati condivisa da tutte le viste a calendario; risolve anche la vecchia voce "Filtrare i dati per ruolo" (la vista di un volontario è naturalmente filtrata sui propri turni/slot liberi).
4. [x] **API: richiesta di prenotazione (volontario)**: `POST /v1/shift-bookings` (nuovo permesso `shifts:request`, assegnato solo a `emergency_volunteer` — non un controllo su `claims.Roles` come ipotizzato in ADR-0025, per restare sull'unico meccanismo di autorizzazione già usato ovunque). Crea una prenotazione "in attesa" per il chiamante stesso (`volunteer_id` dal JWT, non dal body), rifiutando data passata/malformata, template ignoto/non attivo/weekday non corrispondente (400/404), slot già confermato o propria richiesta doppia (409). `GET /v1/shift-bookings/pending-count` (dietro `shifts:write`) espone il conteggio globale delle richieste in sospeso, base del contatore in-app per il gestore turni (voce 11).
5. [x] **API: approvazione/rifiuto delle richieste (gestore turni)**: `PATCH /v1/shift-bookings/{id}` (dietro `shifts:write`), corpo `{"status": "confirmed"|"rejected"}` — qualunque altro valore è 400. `decided_by` preso dal token del gestore, non dal body. Transizione atomica (`WHERE status = 'pending'`): 404 se l'id non esiste, 409 se la prenotazione non è più in attesa (già decisa o annullata) — nessuna doppia decisione silenziosa. Nessuna cascata automatica sulle altre richieste pending per lo stesso slot (limite noto, vedi ADR-0025).
6. [x] **API: prenotazione diretta per un volontario (gestore turni)**: `POST /v1/shift-bookings/direct` (dietro `shifts:write`), corpo `{template_id, volunteer_id, date}` — rotta dedicata, non riusa `POST /v1/shift-bookings` (che ha il contratto opposto: `volunteer_id` dal JWT, non dal body). Stesse validazioni della richiesta del volontario (data/template/weekday, slot già confermato, volontario scelto già prenotato per lo slot), condivise via helper interni. La prenotazione nasce già `confirmed`, `decided_by` è il gestore stesso (dal token, non dal body). Nuovo `volunteer_id` inesistente → 400 pulito (non 500), intercettando la violazione di FK a livello di repository.
7. [x] **API: prenotazioni multiple atomiche (volontario)**: `POST /v1/shift-bookings/bulk` (dietro `shifts:request`), corpo `{"bookings": [{template_id, date}, ...]}`. Lista vuota o con coppie `(template_id, date)` duplicate → 400 subito. Ogni elemento è validato nell'ordine in cui compare con le stesse regole della richiesta singola (voce 4); il primo che fallisce interrompe tutto — **nessuna scrittura in DB se anche un solo elemento non è valido**, errore prefissato con l'indice dell'elemento (es. `"booking 2: date must not be in the past"`). Se tutti passano, un'unica transazione DB (`CreateBookingsAtomic`) inserisce tutte le prenotazioni: se una singola riga fallisce a inserimento, l'intera transazione va in rollback. Tutte nascono `pending`, come la richiesta singola.
8. [ ] **(Bassa priorità, dopo che il flusso in-app funziona) Notifica email al gestore turni**: invio di un'email ad ogni nuova richiesta, portando l'infrastruttura di invio (stile `registry`, ADR-0023) anche in `shifts` — o tramite una chiamata a `registry` — deliberatamente rimandato.
...
- [x] **Invio automatico email di invito alla creazione utente**: inviato via SMTP (Gmail o compatibile) il link di attivazione all'email del nuovo utente, con mittente configurabile per comitato (`config/<slug>/registry/email.json`); opzionale, se SMTP non è configurato il link resta da copiare e inoltrare manualmente come prima. Vedi [ADR-0023](adr/0023-invio-email-invito-smtp.md).
- [x] **Email di recupero password**: flusso forgot-password completo (`POST /v1/password/reset/request` + `/confirm`), riusa il modello token e il mailer esistenti; invio email best-effort come per l'invito, sempre 204 alla richiesta per non rivelare se l'account esiste. Vedi [ADR-0024](adr/0024-recupero-password.md).
...
- [ ] **Filtrare i dati per ruolo** (es. un volontario vede solo i propri turni) — affrontato dalla sezione "Gestione turni" più sopra (voce 3), non più da progettare qui.
