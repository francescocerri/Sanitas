...
3. [ ] **API: calendario e copertura (sola lettura)**: endpoint che elenca le occorrenze di turno in un intervallo di date (generate dai template attivi) con lo stato di copertura (libero / richiesta in attesa / confermato), dietro `shifts:read` — base dati condivisa da tutte le viste a calendario; risolve anche la vecchia voce "Filtrare i dati per ruolo" (la vista di un volontario è naturalmente filtrata sui propri turni/slot liberi).
