...
- [x] **Refresh token**: `POST /v1/login` ora restituisce anche un refresh token (30gg, rotante), più `POST /v1/refresh` e `POST /v1/logout` — riusa la tabella `invite_tokens` esistente. Vedi [ADR-0016](adr/0016-refresh-token.md). Nessun rilevamento di riuso/furto (token family) né revoca dell'access token già emesso — limiti noti, non affrontati.
