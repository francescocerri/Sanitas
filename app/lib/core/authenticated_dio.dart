...
/// Costruisce un client `dio` puntato su [baseUrl] con lo stesso
/// comportamento per ogni servizio autenticato di questa app:
///
...
///    dell'app), prova UNA VOLTA a rinnovarlo (`AuthController.refreshSession`)
///    e ripete automaticamente la richiesta originale con il nuovo token. Se
///    anche il refresh fallisce, l'errore risale al chiamante e
///    `AuthController` avrà già spostato la sessione su "non autenticato".
///
/// Estratta da `api_client.dart` (dove viveva solo per `registry`) perché
/// `shifts_api_client.dart` ha bisogno esattamente della stessa logica
/// contro un `baseUrl` diverso — un solo interceptor testato, non due copie
/// che potrebbero divergere.
Dio createAuthenticatedDio(Ref ref, String baseUrl) {
