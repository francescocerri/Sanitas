...
- **Database**: PostgreSQL self-hosted, un solo container nel `docker-compose` condiviso da tutti i servizi (un database, `sanitas`), con uno **schema Postgres nativo per servizio** (`anagrafica`, `turni`, ...) — non un container/database per servizio. Ogni servizio resta proprietario del proprio schema, creato dal proprio binario al primo avvio via **GORM AutoMigrate** — non più da script SQL in `/docker-entrypoint-initdb.d/` (vedi [ADR-0019](docs/adr/0019-gorm-e-automigrate.md), [ADR-0020](docs/adr/0020-turni-gorm.md)). `turni` applica il proprio con retry, perché la sua FK dipende dallo schema di `anagrafica` già esistente. FK reali tra schemi di servizi diversi sono ammesse quando esprimono un vincolo di dominio reale (es. `turni.turni.volontario_id → anagrafica.users.id`). Vedi [ADR-0014](docs/adr/0014-database-condiviso-schema-separati.md).
