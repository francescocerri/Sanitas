...
- **Prima di ogni `git push`/apertura o aggiornamento di una PR**: rieseguire in locale l'intera suite di verifica (`go build`, `go vet`, `go test`, `gofmt -l`, `govulncheck`, rigenerazione Swagger e controllo che non ci sia drift)
. **Il comando di rigenerazione Swagger deve essere esattamente quello usato in CI** (`.github/workflows/ci.yml`, oggi `go run github.com/swaggo/swag/cmd/swag@v1.16.4 init -g cmd/server/main.go -d . -o api --parseInternal --parseDependency`, da dentro la cartella del servizio): un `swag init` "ragionevole" ma con flag diversi (es. senza `--parseDependency`) genera nomi di tipo diversi (`httpapi.X` invece di `internal_httpapi.X`/nomi col path completo) — sembra allineato in locale ma la CI lo segnala comunque come drift. Causa reale di un fallimento CI ricorrente su questo step, risolta una volta ma da non reintrodurre.
