...
- **Database**: PostgreSQL self-hosted, un solo container nel `docker-compose` condiviso da tutti i servizi (un database, `sanitas`), con uno **schema Postgres nativo per servizio** (`anagrafica`, `turni`, ...) — non un container/database per servizio. Ogni servizio resta proprietario della propria migrazione, applicata via script SQL montati in `/docker-entrypoint-initdb.d/` (feature nativa dell'immagine ufficiale Postgres) — nessun tool di migrazione finché non serve evolvere lo schema in modo incrementale su più release. FK reali tra schemi di servizi diversi sono ammesse quando esprimono un vincolo di dominio reale (es. `turni.turni.volontario_id → anagrafica.users.id`). Vedi [ADR-0014](docs/adr/0014-database-condiviso-schema-separati.md).
