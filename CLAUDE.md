...
```
services/<nome-servizio>/   # shifts, registry (implementati); mezzi-magazzino, servizi-emergenze da fare
...
- **Database**: PostgreSQL self-hosted, un solo container nel `docker-compose` condiviso da tutti i servizi (un database, `sanitas`), con uno **schema Postgres nativo per servizio** (`registry`, `shifts`, ...) — non un container/database per servizio. Ogni servizio resta proprietario del proprio schema, creato dal proprio binario al primo avvio via **GORM AutoMigrate** — non più da script SQL in `/docker-entrypoint-initdb.d/` (vedi [ADR-0019](docs/adr/0019-gorm-e-automigrate.md), [ADR-0020](docs/adr/0020-shifts-gorm.md)). `shifts` applica il proprio con retry, perché la sua FK dipende dallo schema di `registry` già esistente. FK reali tra schemi di servizi diversi sono ammesse quando esprimono un vincolo di dominio reale (es. `shifts.shifts.volunteer_id → registry.users.id`). Vedi [ADR-0014](docs/adr/0014-database-condiviso-schema-separati.md).
...
- **Lingua**: codice, commenti (incluse le annotazioni Swagger), log strutturati (`log/slog` o equivalente), i messaggi nel body delle risposte HTTP (es. `{"error": "invalid credentials"}`, vedi [ADR-0015](docs/adr/0015-messaggi-errore-in-inglese.md), supera la parte di [ADR-0010](docs/adr/0010-convenzioni-cross-cutting-servizi-go.md) che teneva questi messaggi in italiano) sono in inglese — convenzione standard per codice destinato a essere letto/consultato anche da chi non parla italiano. Aggiornamento: questa convenzione copre **tutto il vocabolario di dominio**, non solo i messaggi — i nomi dei campi JSON (es. `volunteer_id`, non `volontario_id`) e i valori di stato/enum (es. `status: "pending"`, non `"pianificato"`) sono anch'essi in inglese, chiudendo l'inconsistenza che restava aperta fino a [ADR-0025](docs/adr/0025-modello-dati-turni.md). I segmenti del path delle route HTTP (es. `/v1/shifts`, `/v1/users`) erano già in inglese — vedi ADR-0010. **Resta in italiano solo la documentazione del progetto** (questo file, `docs/adr/`, `docs/backlog.md`, `docs/funzionale/`) — non il codice/contratto API. Dati puramente di configurazione/branding di un comitato (es. `display_name: "Presidente"` in `roles.json`) non sono vocabolario di dominio: restano nella lingua del comitato che li ha scritti.
...
- **Prima di ogni `git push`/apertura o aggiornamento di una PR**: rieseguire in locale l'intera suite di verifica (`go build`, `go vet`, `go test`, `gofmt -l`, `govulncheck`, rigenerazione Swagger e controllo che non ci sia drift) — non fidarsi solo della CI remota, verificarlo anche in locale prima del push. **Il comando di rigenerazione Swagger deve essere esattamente quello usato in CI** (`.github/workflows/ci.yml`, oggi `go run github.com/swaggo/swag/cmd/swag@v1.16.4 init -g cmd/server/main.go -d . -o api --parseInternal --parseDependency`, da dentro la cartella del servizio): un `swag init` "ragionevole" ma con flag diversi (es. senza `--parseDependency`) genera nomi di tipo diversi (`httpapi.X` invece di `internal_httpapi.X`/nomi col path completo) — sembra allineato in locale ma la CI lo segnala comunque come drift. Causa reale di un fallimento CI ricorrente su questo step, risolta una volta ma da non reintrodurre.
