...
// production package imports this (see docs/adr/0011 in services/shifts,
...
func StartPostgres(ctx context.Context, migrate func(*gorm.DB) error) (*gorm.DB, func(), error) {
	...
	// search_path=registry: Migrate creates the tables inside an
	// "registry" schema, not the database's default public one (the two
	// services share one database — see docs/adr/0014) — this keeps
	// existing unqualified queries (FROM users, no schema prefix) working.
	connString, err := container.ConnectionString(ctx, "sslmode=disable", "search_path=registry")
	if err != nil {
